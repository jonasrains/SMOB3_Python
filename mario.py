...
def touch_semisolid(world, level):
    for i in data['semisolid'][str(world) + '-' + str(level)]:
        collide = collision.collision([[xpos, xpos + 28], [ypos + 52, ypos + 54]], i)
        if collide[0]:
            return collide
    return [False, []]
...
def touch_ceiling(world, level):
    return collision.collision([[xpos, xpos + 28], [ypos, ypos + 2]], data['ground'][str(world) + '-' + str(level)])
    ...
    if jumping and jump_rep < jump_height + 2:
        ...
        if jump_rep < jump_height:
            ...
            for i in range(round(abs((jump_rep - jump_height)/2))):
                if not touch_ceiling(world, level)[0]:
                    ypos -= ((0-(abs(jump_rep - jump_height) / (jump_rep - jump_height)))*4)
...
def update(left_pressed, right_pressed, up_pressed, level, world):
    global image, frame, walk_anim, velocity, frame, direction, xpos, ypos, falling
    falling = False
